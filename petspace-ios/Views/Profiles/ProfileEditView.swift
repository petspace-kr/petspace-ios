...
struct ProfileEditView: View {
    ...
    @State private var previousDogName: String = ""
    @State private var previousDogBreed: String = ""
    @State private var previousDogSize: DogSize = .small
    @State private var previousDogWeight: String = ""
    ...
    // 견종
    let dogBreedData: DogBreed = load("dogbreed.json")
    ...
    var body: some View {
                    ...
                    if isLoading {
                        ...
                        Button("저장") {
                            ...
                            if let dogWeightDouble = Double(dogWeight) {
                                print("changed dogname:" + dogName)
                                profileViewModel.updateProfile(index: selectedIndex, dogName: dogName, dogBreed: dogBreed, dogSize: dogSize, dogWeight: dogWeightDouble, profileImage: selectedImage)
                                
                                dismiss()
                                isPresented = false
                            }
                        }
                        .disabled((dogName == "" || dogBreed == "" || dogWeight == "" || Double(dogWeight) == nil) || (dogName == previousDogName && dogBreed == previousDogBreed && dogWeight == previousDogWeight && dogSize == previousDogSize))
                    }
        ...
        // load profile data
        .onAppear {
            let loadProfile: DogProfile = profileViewModel.dogProfile[selectedIndex]
            dogName = loadProfile.dogName
            dogSize = loadProfile.dogSize
            dogBreed = loadProfile.dogBreed
            dogWeight = "\(loadProfile.dogWeight)"
            dogBreedIndex = 999
            
            // save previous profile data
            previousDogName = dogName
            previousDogSize = dogSize
            previousDogBreed = dogBreed
            previousDogWeight = dogWeight
            
            if let imageData = loadProfile.profileImageData {
...
#Preview {
    ...
    return Group {
        NavigationStack {
            ProfileEditView(isPresented: .constant(true), profileViewModel: profileViewModel, selectedIndex: .constant(0))
        }
    }
}
